...
public final class CleopatraClient {
    ...
    public static void register(EntityRenderersEvent.RegisterRenderers e) {
        e.registerEntityRenderer(CleopatraEntities.BOSS.get(), BossRenderer::new);
        e.registerEntityRenderer(CleopatraEntities.SNAKE_POISON.get(), c -> new SnakeRenderer(c, "cleopatra_snake_poison"));
        e.registerEntityRenderer(CleopatraEntities.SNAKE_FIRE.get(), c -> new SnakeRenderer(c, "cleopatra_snake_fire"));
        e.registerEntityRenderer(CleopatraEntities.SNAKE_ICE.get(), c -> new SnakeRenderer(c, "cleopatra_snake_ice"));
        e.registerEntityRenderer(CleopatraEntities.SANDWORM.get(), SandwormRenderer::new);
        e.registerEntityRenderer(CleopatraEntities.SCORPION.get(), ScorpionRenderer::new);
        e.registerEntityRenderer(CleopatraEntities.VENOM_BULLET.get(), c -> new FlatRenderer<>(c, new ResourceLocation("yellowduck", "textures/entity/elite_venom_bullet.png"), .35f, false));
        e.registerEntityRenderer(CleopatraEntities.VENOM_POOL.get(), c -> new FlatRenderer<>(c, new ResourceLocation("yellowduck", "textures/entity/elite_pool_purple.png"), 2f, true));
        e.registerEntityRenderer(CleopatraEntities.VENOM_RING_FIRE.get(), RingRenderer::new);
        e.registerEntityRenderer(CleopatraEntities.VENOM_RING_ICE.get(), RingRenderer::new);
        e.registerEntityRenderer(CleopatraEntities.VENOM_RING_PLAIN.get(), RingRenderer::new);
        e.registerEntityRenderer(CleopatraEntities.BOMB_MARK_POISON.get(), BombRenderer::new);
        e.registerEntityRenderer(CleopatraEntities.BOMB_MARK_BURNING.get(), BombRenderer::new);
        e.registerEntityRenderer(CleopatraEntities.BOMB_MARK_FROZEN.get(), BombRenderer::new);
        e.registerEntityRenderer(CleopatraEntities.SNAKE_SUMMONER.get(), NoRenderer::new);
    }

    static class BossRenderer extends GltfEntityRenderer<CleopatraBoss> {
        ...
        @Override
        protected float renderYawDegrees(CleopatraBoss entity, float entityYaw, float partialTick) {
            return 0.0F;
        }
        ...
        public void render(CleopatraBoss x, float y, float p, PoseStack ps, MultiBufferSource b, int l) {
            ...
            ps.translate(BBOX_CENTER_X * MODEL_SCALE, 0.0D, BBOX_CENTER_Z * MODEL_SCALE);
            super.render(x, 0.0F, p, ps, b, l);
            ps.popPose();
        }
    }

    static class SnakeRenderer extends GltfEntityRenderer<CleopatraVenomSnake> {
        private static final float MODEL_SCALE = .1f;
        ...
        @Override
        protected float renderYawDegrees(CleopatraVenomSnake entity, float entityYaw, float partialTick) {
            return 0.0F;
        }
    ...
    static class FlatRenderer<T extends Entity> extends EntityRenderer<T> {
        ...
        FlatRenderer(EntityRendererProvider.Context c, ResourceLocation t, float s, boolean h) {
            super(c);
            tex = t;
            size = s;
            horizontal = h;
